...
export function ProjectDetailForm({ project }: { project: Project }) {
  ...
  const [form, setForm] = useState({
    ...
    owed: String(project.owed),
    advance: String(project.advance),
    project_expenses: String(project.project_expenses ?? 0),
  });
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        project_expenses: Number(form.project_expenses) || 0,
      })
      .eq("id", project.id);
  ...
  const mainFields: { label: string; key: keyof typeof form; type?: "text" | "number"; step?: string }[] = [
    { label: "Όνομα πελάτη", key: "customer_name" },
    { label: "Τιμή ανά μέτρο", key: "price_per_meter", type: "number", step: "0.01" },
    { label: "Σιναζι", key: "sinazi" },
    { label: "Γωνίες", key: "gonies" },
    { label: "Οφειλή", key: "owed", type: "number", step: "0.01" },
    { label: "Προκαταβολή", key: "advance", type: "number", step: "0.01" },
    { label: "Εξοδα εργου", key: "project_expenses", type: "number", step: "0.01" },
  ];
    ...
    <form onSubmit={handleSubmit} className="space-y-0">
      <div className="rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <dl className="divide-y divide-zinc-200 dark:divide-zinc-800">
          {mainFields.map(({ label, key, type = "text", step }) => (
            <div
              key={key}
          ...
          {/* Έσοδα: λίστα Ποσό + ΦΠΑ % (προαιρετικό), υπολογιζόμενο σύνολο ανά γραμμή και συνολικά */}
            ...
            <dd className="space-y-3 text-sm sm:col-span-2">
              ...
              {incomeRows.length > 0 && (
                <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                  Σύνολο έσοδα: €{incomeTotal.toLocaleString("el-GR", { minimumFractionDigits: 2 })}
                </p>
              )}
            </dd>
          </div>
